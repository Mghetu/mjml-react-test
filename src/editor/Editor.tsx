...
import { registerAnchorPlugin } from './plugins/anchorPlugin';
import { registerMjImageAnchorFix } from './plugins/mjImageAnchorFix';
...
export default function Editor() {
  ...
  const handleEditorReady = useCallback((editor: GrapesEditor) => {
    ...
    registerMjImageAnchorFix(editor);
    ...
    const formatMjmlErrors = (errors: unknown) => {
    ...
    const addTemplateActions = () => {
      ...
      const existingPanel = Panels.getPanel(panelId);
      if (!existingPanel) {
        Panels.addPanel({ id: panelId, visible: true });
      }

      editor.Commands.add('export-mjml-template', {
        run(currentEditor) {
          const mjmlMarkup = currentEditor.getHtml();
          downloadFile(mjmlMarkup, 'template.mjml', 'application/vnd.mjml+xml');
        },
      });

      editor.Commands.add('convert-mjml-to-html', {
        async run(currentEditor) {
          const mjmlMarkup = currentEditor.getHtml();
          ...
          try {
            ...
            if (!response.ok) {
              if (response.status === 400 && payload?.errors) {
                alert(formatMjmlErrors(payload.errors));
                return;
              }
            ...
            if (!payload || typeof payload.html !== 'string') {
              ...
              alert('Unexpected response from MJML conversion service.');
              return;
            }
      ...
      Panels.addButton(panelId, {
        id: 'export-mjml-template',
        command: 'export-mjml-template',
        label: 'Export MJML',
        className: 'gjs-pn-btn',
        attributes: { title: 'Download current MJML' },
      });

      Panels.addButton(panelId, {
        id: 'convert-mjml-to-html',
        command: 'convert-mjml-to-html',
        label: 'Convert MJML to HTML',
        className: 'gjs-pn-btn',
        attributes: { title: 'Compile MJML to HTML via backend service' },
      });
    };
